...
The development compose file [`docker-compose.dev.yml`](./docker-compose.dev.yml)
uses named volumes (`db-data`, `media-data`, `logs-data`) to keep data between
container rebuilds. Even though the repo is bind-mounted to `/app`, the
`media-data` named volume is mounted at `/app/media`, so any videos generated by
the app are stored **inside** the running container (not in your local
`./media` folder).

To copy a video out of the container, run a command like:

```bash
docker compose cp app:/app/media/video/<file> ./my-videos/
```

Replace `<file>` with the actual video filename. The service name is `app`, and
`docker compose ps` will show the full container name (for example,
`dream-recorder-app-1`) if you prefer to use `docker cp` directly.

If you prefer to access the media directory directly from the host, edit the
`volumes` section of `docker-compose.dev.yml` and mount a host path instead of
the named volume:
...
After updating the compose file, restart the stack with `docker compose down`
