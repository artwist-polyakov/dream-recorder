...
container rebuilds. The `media/` folder from the project is mounted to the
`media-data` volume, so any videos generated by the app are stored **inside** the
running container.
...
Replace `<file>` with the actual video filename. The service name is `app`, so
you can see its container with `docker compose ps`.
